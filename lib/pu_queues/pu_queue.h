...
/*
    ...
    Contains circular thread protected queue implementation. Multiple async reads/writes allowed.
...
/* Special predefined event to pass STOP signal to threads connected to the queue (Never been used and/or tested) */
 ...
 *
 * @param queues_amount - max queues amount
 * @return  - 1 if OK, 0 if not
*/
int pu_queues_init(unsigned int queues_amount);

/**
 * Destroy queues. Should be called at the very end of process
*/
 ...
 *
 * @param queue_events_mask - set of events
 * @param event             - new event added
 * @return  - event set with new event added
 */
 ...
 *
 * @param queue_events_mask - set of events could come
 * @param to_sec            - timeout in seconds
 * @return  - event happens. If the timeout exceeds - PQ_TIMEOUT came.
*/
 ...
 *
 * @param records_amt   - max records amount in queue
 * @param my_event      - the event associated with the queue. When the data arrives, the evend issued.
 ...
 * Dispose the queue
 *
 * @param queue - pointer to the queue to be erased
 */
 ...
 *
 * @param data  - new element added
 * @param len   - data length. (strlen(data)+1)
*/
 ...
 *
 * @param queue - the data source queue
 * @param data  - buffer for queue element returned
 * @param len   - data buffer length NB! this is IO parameter: the actual data lenght will be returned
 ...
 *
 * @param queue - the checking queue
