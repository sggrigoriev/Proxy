 ...
 *  limitations under the License.
 ...
 Contains Proxy commands & alerts internal representation + conversion to/from JSON utilities
 ...
 *
 * @param child_name    - the value mapped on pr_child_t enums
 * @param name          - process name to be stored
 ...
 *
 * @param child_name    - process index
 * @return  - empty string if error or the process name
*/
 ...
 *
 * @param child_name    - process name
 * @return  - PR_CHILD_SIZE if error or process index if Ok
*/
 ...
 *
 * @param item  - process index to be checked
 * @return  - 1 if OK, retirn 0 id item < 0 or > max index
*/
int pr_child_t_range_check(int item);
/*
Messaging part
Message type definition - just to hide the JSON from all other places of use
*/
...
P   R_CMD_REBOOT           - command for reboot        (watchdog->WUD)
*/
...
/* Here is the list of commands sent by cloud. This is needed to separate internal ones from cloud */
static const pr_cmd_t cloud_commands[] = {
        PR_CMD_FWU_START,
        PR_CMD_FWU_CANCEL,
...
/**
 * Parse JSON string to JSON object. NB! The object should be deleted!
 *
 * @param msg   - JSON string
 * @return  - JSON object or NULL if parsing error
*/
 ...
 * Erase JSON object
 *
 * @param msg   - JSON object to be deleted
*/
 ...
 *
 * @param array - suppose to be array
 * @return  - 0 if array is empty or the object is not an array or array size
*/
 ...
 * @param idx   - the index
 * @return  - NULL if error or i-th object from array
*/
 ...
 *
 * @param msg      - JSON object to be converted
 * @param text_msg - the buffer for the converted string
...
/**
 * Command type recognition by command item - parameters mostly.
 * The command defines by its parameters... New experience for me. But very modern one
 *
 * @param cmd_item  - element from commands array.
 * @return  - PR_CMD_UNDEFINED if error or valid command type
*/
pr_cmd_item_t pr_get_cmd_item(msg_obj_t* cmd_item);

/**
 * This function splits the commands array into Proxy-related and Agent-related sub-arrays.
 * For now these commands sets are not intersects which is great
 *
 * @param msg               - original message as JSON object
 * @param device_id         - gateway device id - one of ways how to understand the command belongs to Proxy, not ot the end-device
 * @param msg4proxy         - buffer for commands to Proxy converted to the JSON string. Could be "" if no commands for Proxy
 * @param msg4proxy_size    - buffer size
 * @param msg4agent         - buffer for commands to Agent converted to the JSON string. Could be "" if no commands for Agent
 * @param msg4agent_size    - buffer size
*/
 ...
 *
 * @param msg   - message as JSON object
 * @return  - message type (command or alert or other).
*/
...
/**
 * Create the connection info command (PR_CMD_CLOUD_CONN) (Proxy->WUD, Proxy ->Agent)
 *
 * @param buf       - buffer to save the command
 * @param size        - buffer size
 * @param conn_string - cloud connection URL
 * @param device_id   - gateway device id
 * @param auth_token  - gateway authentication token
 * @param version     - gateway firmware version
 ...
 * @param size          - buffer size
 * @param status        - firmware upgrade status
 * @param fw_version    - gateway current firmware version
 * @param device_id     - gateway device id
 ...
 *
 * @param buf               - buffer for the message created               -
 * @param size              - buffer size
 ...
 * @param buf       - buffer to store the message
 * @param size      - buffer size
 * @param m_alert   - alert body
 * @param device_id - gateway device id
 * @param val       - reboot status (see pr_reboot_param_t)
 * @return  - pointer to buf
*/
...
/**
 * Create watchdog alert (Proxy->WUD, Agent->WUD)
 *
 * @param buf       - buffer to store the message
 * @param size      - buffer size
 * @param component - sender's process name
 * @param device_id - gateway device_id
 * @return  - pointer to the buf
*/
 ...
 *
 * @param alert_item    - alert item as JSON object
 * @return  - alert type
*/
pr_alert_item_t pr_get_alert_item(msg_obj_t* alert_item);

/**
 * Recognize the message {"agent_request": "connect"}
 *
 * @param obj   - parsed message
 * @return  - 1 if the obj contains this message, 0 if not
 */
int pr_agent_connected(msg_obj_t* obj);

/**
 * Cloud command recognition
 *
